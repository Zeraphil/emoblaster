...
namespace ShapeBlaster
{
	static class EntityManager
	{
		...
		public static void Update(Single s, Single skill)
		{
			isUpdating = true;
			HandleCollisions();
            UpdateScale(s);
            ...
            if (skill > 0.5f)
                if (PlayerShip.Instance.IsArmed)
                    Superweapon();
        ...
        static void UpdateScale(Single s)
        {
            for (int i = 0; i < enemies.Count; i++)
            {
                enemies[i].HandleScaling(s);
            }
        }
		...
		private static void KillPlayer()
		{
			...
			enemies.ForEach(x => x.Kill());
			blackHoles.ForEach(x => x.Kill());
			EnemySpawner.Reset();
		}

        private static void Superweapon()
        {
            PlayerShip.Instance.Superweapon();
            enemies.ForEach(x => x.Kill());
            blackHoles.ForEach(x => x.Kill());
            EnemySpawner.Reset();
        }
